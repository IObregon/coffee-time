...
app.get('/api/gastoHoy/:ID', function(req, res, next) {
  Persona.findOne({
    ...
    Gasto.findOne({
      _id: persona.gastos[persona.gastos.length - 1]
    }).populate("consumicion").populate("consumicion2").exec(function(err, result) {
      if (err) {
        res.next(err);
      }
      if (result) {
...
app.put('/api/gasto/:idGasto', function(req, res, next) {
  ...
  Gasto.findOne({
    _id: req.params.idGasto
  }, function(err, gastoViejo) {
    if (err) next(err);
...
function restoreBalance(gastoNuevo, cb) {
  Persona.findOne({
    _id: gastoNuevo._creador
  }, function(err, creador) {
